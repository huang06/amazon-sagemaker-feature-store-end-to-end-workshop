...
class FeatureStore:
    ...
    _iam_pipeline_start_policy = {
        "Version": "2012-10-17",
        "Statement": [
            {"Effect": "Allow", "Action": ["sagemaker:StartPipelineExecution"], "Resource": ["TBD"]}
        ],
    }
    _iam_trust_policy = {
        'Version': '2012-10-17',
    ...
    def __init__(self):
        ...
        self._sm_client = self._boto_session.client(service_name='sagemaker', region_name=self._region)
        self._featurestore_runtime = self._boto_session.client(
            service_name='sagemaker-featurestore-runtime', region_name=self._region
        )
    ...
    def list_feature_groups(self, name_contains: Optional[str] = None) -> List[str]:
    ...
    def _create_script(
        self, out_script: str, user_defined_script: Optional[str] = None, script_type: str = 'python'
    ) -> None:
        ...
        if script_type == 'python':
            ...
            filenames = [
                f'{self._script_dir}/batch_ingest_sm_sklearn_top.py',
                _processing_script,
            ...
            filenames = [
                f'{self._script_dir}/batch_ingest_sm_pyspark_top.py',
                _processing_script,
    ...
    def schedule_feature_pipeline(
        ...
        s3_uri_prefix: str,
        fg_name: str,
        user_defined_script: Optional[str] = None,
        ...
        if script_type == 'python':
            ...
            processor = PySparkProcessor(
                ...
                instance_count=instance_count,
                base_job_name=pipeline_name,
                env={'AWS_DEFAULT_REGION': self._region, 'mode': 'python'},
        ...
        #     RoleName=event_bridge_role['Role']['RoleName'], PolicyArn=policy_arn
        # )
        ...
        events_client.put_targets(
            Rule=rule_name,
            Targets=[{'Id': tmp_id, 'Arn': pipeline_arn, 'RoleArn': event_bridge_role['Role']['Arn']}],
        )
    ...
    def delete_feature_group(self, fg_name: str, delete_s3: bool = True) -> None:
        ...
        if delete_s3 and has_offline_store:
            ...
            offline_prefix = (
                f'{base_offline_prefix}/{self._account_id}/sagemaker/'
        ...
        try:
            self._sm_client.delete_feature_group(FeatureGroupName=fg_name)
        except:
            pass
    ...
    def _wait_for_feature_group_deletion_complete(self, feature_group_name: str) -> None:
        ...
        while True:
            try:
                self.describe_feature_group(feature_group_name)['FeatureGroupStatus']
    ...
    def create_fg_from_df(
        ...
        id_name: str = 'Id',
        event_time_name: str = 'UpdateTime',
        ...
        if id_name not in df.columns:
            ...
            return
        if event_time_name not in df.columns:
    ...
    def ingest_from_df(
        self, fg_name: str, df: pd.DataFrame, max_processes: int = 4, max_workers: int = 4, wait: bool = True
    ) -> None:
    ...
    def delete_record(self, fg_name: str, record_id: Union[str, int], event_time: Union[str, int]) -> None:
        ...
        resp = self._featurestore_runtime.delete_record(
            FeatureGroupName=fg_name, RecordIdentifierValueAsString=str(record_id), EventTime=event_time
        )
        return
    ...
    def _record_to_dict(
        ...
        tmp_dict: Dict[str, Union[str, float]] = {}
    ...
    def get_latest_feature_values(
        self, fg_name: str, identifiers: List[Union[str, int]], features: Optional[List[str]] = None
        ...
        for curr_id in identifiers:
            ...
            if features is None:
                resp = self._featurestore_runtime.get_record(
                    FeatureGroupName=fg_name, RecordIdentifierValueAsString=record_identifier_value
                )
            else:
                resp = self._featurestore_runtime.get_record(
                    FeatureGroupName=fg_name,
    ...
    def _get_core_features(self, fg_name: str) -> List[str]:
        ...
        _cleaned_list = [x for x in _feature_list if x not in _exclude_list]
    ...
    def _feature_list_to_df(self, features: List[str]) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            _groups_and_features, columns=['fg_name', 'feature_name', 'fg_number', 'feature_number']
        )

    def get_latest_featureset_values(
        self, id_dict: Dict[str, Union[str, int]], features: List[str], verbose: bool = False
        ...
        for _g in _gb:
            ...
            if verbose:
                print(
                    f'fg name: {_fg_name}, id: {_id_feature_name}, id val: {_id_value}, features: {_curr_features}'
                )

            _fg_requests.append(
                {
                    'FeatureGroupName': _fg_name,
        ...
        _results_dict: Dict[str, Union[str, float]] = {}
        ...
        for _req in _fg_requests:
            ...
            _curr_record = next(_item for _item in _all_records if _item['FeatureGroupName'] == _fg_name)[
                'Record'
            ]
            if verbose:
                print(_curr_record)
    ...
    def _run_query(
        self, query_string: str, tmp_uri: str, database: str, verbose: bool = False
    ) -> pd.DataFrame:
        ...
        query_state = athena.get_query_execution(QueryExecutionId=query_execution_id)['QueryExecution'][
            'Status'
        ]['State']
        ...
        if query_state == 'FAILED':
            ...
            failure_reason = athena.get_query_execution(QueryExecutionId=query_execution_id)[
                'QueryExecution'
    ...
    def _get_offline_details(self, fg_name: str, s3_uri: Optional[str] = None) -> Tuple[str, str, str]:
    ...
    def sample(
        self, fg_name: str, n: int = 10, sample_pct: int = 25, s3_tmp_uri: Optional[str] = None
    ) -> pd.DataFrame:
        ...
        _query_string = 'SELECT * FROM "' + _table + '"' + f' tablesample bernoulli({sample_pct}) limit {n}'
        return self._run_query(_query_string, _tmp_uri, _database, verbose=False)

    def get_minmax_timestamps(self, fg_name: str, s3_tmp_uri: Optional[str] = None) -> pd.DataFrame:
        ...
        _query_string = (
            'SELECT min(write_time) as min_write_time, max(write_time) as max_write_time, '
    ...
    def query(self, fg_names: str, query: str, s3_tmp_uri: Optional[str] = None) -> pd.Dataframe:
        _query_string = query
    ...
    def get_historical_record_count(self, fg_name: str, s3_tmp_uri: Optional[str] = None) -> int:
        ...
        """
        _table, _database, _tmp_uri = self._get_offline_details(fg_name, s3_tmp_uri)
        _query_string = 'SELECT COUNT(*) FROM "' + _table + '"'
        _tmp_df = self._run_query(_query_string, _tmp_uri, _database, verbose=False)
    ...
    def _id_list_as_string(self, fg_name: str, record_ids: List[Union[str, int]]) -> str:
        ...
        id_feature_def = [
            dictionary for dictionary in feature_defs if dictionary['FeatureName'] == id_feature_name
        ]
    ...
    def _get_other_cols(
        self, events_df: pd.DataFrame, events_timestamp_col: str, fg_desc: Dict, features: List[str]
    ) -> str:
        ...
        _cleaned_list = [x for x in _col_list if x not in _exclude_list]
    ...
    def _get_athena_col_defs(self, df: pd.DataFrame) -> str:
        _col_num = 0
    ...
    def _make_temp_table(
        self, tmp_table_name: str, csv_location: str, events_df: pd.DataFrame, verbose: bool = False
    ) -> str:
        athena_cols = self._get_athena_col_defs(events_df)
        serde_properties = '"separatorChar" = ",", "quoteChar"     = "`", "escapeChar"    = "\\\\"'

        _create_string = (
            f'CREATE EXTERNAL TABLE {tmp_table_name} ({athena_cols}) '
    ...
    def _row_level_time_travel(
        ...
        _sub_query = (
            f'select f.{_id_feature_name}, {_f_cols}, '
        ...
        _travel_string = (
            f'with temp as (select {_id_feature_name}, {_other_cols}, '
        ...
        if verbose:
            print(
                f'== Completed row-level travel on {fg_table_name}, result shape: {0 if df is None else df.shape}'
            )
        return df
    ...
    def _mp_row_level_time_travel_wrapper(request: List) -> pd.DataFrame:
        ...
        return _tmp_fs._row_level_time_travel(
    ...
    def _row_level_time_travel_wrapper(self, request: List) -> pd.DataFrame:
        ...
        return self._row_level_time_travel(
            request[0], request[1], request[2], request[3], request[4], request[5], request[6]
        )
    ...
    def get_features(
        ...
        self._make_temp_table(_events_table_name, _csv_location, events_df, verbose=verbose)
        ...
        for _curr_results_df in _return_data:
            ...
            _curr_results_df = _curr_results_df.drop(
                [_curr_results_df.columns[0], _curr_results_df.columns[1], TEMP_TIME_TRAVEL_ID_NAME], axis=1
            )
            if _cumulative_features_df is None:
                ...
                _cumulative_features_df = pd.concat([_cumulative_features_df, _curr_results_df], axis=1)
                if verbose:
    ...
    def get_historical_offline_feature_values(
        self,
        fg_name: str,
    ...
    def get_latest_offline_feature_values(
        self,
        fg_name: str,
        record_ids: Optional[List[Union[str, int]]] = None,
        feature_names: Optional[List[str]] = None,
        ...
        _subquery = (
            f'SELECT *, ROW_NUMBER() OVER (PARTITION BY {id_feature_name} '
        ...
        _query_string = (
            f'SELECT {feature_name_string} FROM ({_subquery}) WHERE row_num = 1 AND NOT is_deleted'
        )

        return self._run_query(_query_string, _tmp_uri, _database, verbose)

    def get_latest_offline_feature_values_as_of(
        self,
        fg_name: str,
        as_of: str,
        record_ids: Optional[List[Union[str, int]]] = None,
        ...
        if record_ids is None:
            ...
            where_clause = (
                f" WHERE {id_feature_name} IN ({id_list_string}) AND {time_feature_name} <= '{as_of.upper()}'"
            )
    ...
    def _create_processing_inputs(self, processing_dir, flow, flow_uri):
        ...
        for node in flow["nodes"]:
            if "dataset_definition" in node["parameters"]:
                ...
                if source_type == "S3":
                    s3_processing_input = _create_s3_processing_input(processing_dir, name, data_def)
    ...
    def ingest_with_dw(
        ...
        if region == 'us-east-1':  # Name "region" is not defined
            container_uri = (
                "663277389841.dkr.ecr.us-east-1.amazonaws.com/sagemaker-data-wrangler-container:1.3.1"
            )
        elif region == 'us-east-2':  # Name "region" is not defined
            container_uri = (
                "415577184552.dkr.ecr.us-east-2.amazonaws.com/sagemaker-data-wrangler-container:1.3.0"
            )
